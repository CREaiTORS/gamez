...
describe("GameService", () => {
  ...
  test("should determine if the game is complete", () => {
    ...
    gameService.setCurrLevel(2);
    expect(gameService.isGameComplete()).toBe(true);
  });
  ...
  test("should preload assets", async () => {
    ...
    await gameService.preloadAssets();
  ...
  test("should warn when collecting report without ending session", () => {
    const warnSpy = (gameService["warn"] = jest.fn());
    gameService.startSession();
    gameService.collectReport();
    expect(warnSpy).toHaveBeenCalledWith("Cannot collect report, session is still active");
  });
});
